...
> Working draft. Replace the submission fields and the bracketed evidence requests before PDF export. This draft describes the checked-out `main` branch as of 22 September 2026. "Implemented" refers to code in the repository and does not prove that a feature is configured or working on the public production deployment.
...
| Document version | Draft v0.2 · 22 September 2026 |
...
- As a user, I can tell the planner that I wore, liked or rejected an outfit and optionally choose a reason. Feedback is stored, but the current outfit prompt does not retrieve that feedback to change later ranking. This is a known gap, not a shipped learning claim.
...
The database schema stores profiles, wardrobe items, outfit requests and recommendations, feedback, purchase evaluations and Explore feed selections under a user ID. These structures provide a foundation for personalisation. Today, outfit requests retrieve the user's confirmed wardrobe and profile preferences. Feedback is stored but is not yet used by the outfit route, and the product has no demonstrated switching-cost or retention data. The credible current claim is therefore a moat hypothesis based on accumulated user context, not a proven network effect or model-training advantage.
...
The codebase currently includes account registration and sign-in, preference onboarding, a private wardrobe, image analysis, editable garment attributes, outfit planning, outfit feedback capture, purchase evaluation, a curated Explore feed, a public landing page and analytics instrumentation. Scope prioritised workflows that depend on persistent identity and wardrobe context.

The application accepts JPEG, PNG, WebP and HEIC images up to 8 MB on the analysis and purchase routes. Clothing analysis returns structured attributes such as category, colour, pattern, formality, layering role, weather tags and uncertainty notes. The user reviews the result before saving. Outfit requests receive the signed-in user's confirmed wardrobe and selected profile fields. Purchase evaluation extracts attributes, computes a simple category-and-colour similarity score in application code, then asks the model for a structured explanation.

The product roadmap should prioritise completing and validating these end-to-end flows on production, correcting any deployment issues, and measuring evaluation and analytics outcomes. Future work may include using saved feedback in later outfit ranking, improving duplicate matching, bulk upload and size-chart support. These are future items. A sizing-feature pull request exists on `chian/sizing-feature`; it is not merged into the reviewed `main` branch and is not described as available. The initial concept's swipeable daily cards, social sharing and complete-look inspiration feed are also not part of the current implementation.

## M6 · Monetisation, pricing and economics

The landing page presents a free beta and a planned Wearabouts Plus price of S$8.90 per month. It explicitly says the paid tier is not available to purchase. The application has no payment or subscription enforcement in the inspected code. The report therefore describes a pricing proposal, not a launched plan.
...
**Draft tagline:** Make more outfits from the clothes you own.

**Draft description:** Wearabouts turns your wardrobe into a private outfit planner. Add and confirm your clothes, get occasion ideas shaped by Singapore's forecast, and compare a potential purchase with what you already own.

**Draft maker comment:** Hi Product Hunt. We built Wearabouts for the moment you are looking at a full wardrobe and still cannot decide what to wear.
