...
- **The model step has no repair retry.** Section 1.1 said to retry once with a repair instruction. The rules already give a complete, explained fallback, so a second call would cost free-tier quota for little gain. A failed or invalid answer goes straight to the rules' top 3, and `generator` records which one shipped. The call times out after 8 seconds. The prompt is `DAILY_PROMPT_VERSION` in `src/lib/outfits/daily-prompt.ts`.
- **The model sees candidate numbers, not item ids.** It returns `{ candidate, explanation }` pairs checked by `dailyPicksSchema` and `applyDailyPicks`. So it cannot add, drop or invent a piece, and the warnings from the rules are always kept.
- **The streak counts wear days, as open question 4 recommends.** It counts consecutive Singapore days with a `wore` response from any source. Today counts once something is worn. Until then, yesterday's streak still stands, which acts as the grace rule. No column stores it; the route computes it from the last 60 days of feedback.
- **Swipes and buttons.** Swipe left or the X button skips, swipe right or the bookmark saves. On a card that is already saved, a swipe right just moves on. Left and right arrow keys move between cards without recording anything, which matches section 1.3.
