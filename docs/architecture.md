...
All Gemini access goes through `app/src/lib/ai/gemini.ts`. `GOOGLE_GENERATIVE_AI_API_KEY` is paid and is used whenever a route sends a user's photo. The outfit planner tries `GOOGLE_GENERATIVE_AI_FREE_API_KEY_1`, `_2` and `_3` in order when a project reports a quota or capacity failure, then uses `GOOGLE_GENERATIVE_AI_RAG_API_KEY` as its final free fallback. Daily Outfits and My Style use the first free key, while Explore uses the RAG key directly. No free-tier call falls back to the paid key.
