...
`GOOGLE_GENERATIVE_AI_API_KEY` is the paid Google project. It powers photo analysis, purchase evaluation, the Beautify image edit and the sizing screenshot reader. `GOOGLE_GENERATIVE_AI_FREE_API_KEY_1`, `_2` and `_3` are unbilled projects used by the outfit planner in that order. Daily Outfits and My Style use the first key. `GOOGLE_GENERATIVE_AI_RAG_API_KEY` powers Explore and is the outfit planner's fourth free fallback. None of these workflows fall back to the paid key; a route reports a configuration error instead. The legacy `GOOGLE_GENERATIVE_AI_FREE_API_KEY` name is still accepted as the first key for existing deployments. `GEMINI_MODEL`, `GEMINI_RAG_MODEL` and `GEMINI_IMAGE_MODEL` override the default models.
